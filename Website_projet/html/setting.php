...
<main>
	...
	<h1 class="settingh1"> Modify your account data </h1>
	 <form class="oui" action="edit.php" method="post">
      <div class="form_div">
        <label>New emai?:</label>
        <input class="field_class" name="login" type="text" placeholder=<?php echo $_SESSION["email"]; ?> required autofocus>
				<?php if (isset($_COOKIE["email_edit_error"])) {echo '<div style="font-size:14px;color:red;text-align:center"> <p>This email is already taken</p> </div>';
				setcookie("email_edit_error", "", time() - 3600);} ?>
				<label>New Pseudo?:</label>
        <input class="field_class" name="pseudo" type="text" placeholder=<?php echo $_SESSION["pseudo"]; ?> required autofocus>
        ...
        <input id="pass" class="field_class" name="password2" type="password" placeholder="New password"required>
				<?php if (isset($_COOKIE["password_edit_error2"])) {echo '<div style="font-size:14px;color:red;text-align:center"> <p>Please choose the same password</p> </div>';
        ...
        <button class="bouton" type="submit" name="Modifier">Go!</button>
				<?php if (isset($_COOKIE["password_edit_error"])) {echo '<div style="font-size:14px;color:red;text-align:center"> <p>Please enter the right password</p> </div>';
